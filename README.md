...
从**招商银行交易流水 PDF** 中提取结构化交易记录，一键导出为 Excel。深度适配招商银行交易流水 PDF，实测中文还原率显著优于 tabula（详见[对比评测](#与-tabula-的对比评测)）。


## 功能特性

- 自动逆向解析 PDF 字体编码，正确还原中文
- 提取记账日期、币种、交易金额、联机余额、交易摘要、对手信息等字段
...
[tabula](https://github.com/tabulapdf/tabula) 是基于 PDFBox 的通用 PDF 表格提取工具。我们以同一份 60 页招商银行交易流水 PDF（含 1400+ 笔交易）为样本，对比两者的提取效果如下：
...
xychart-beta
    title "中文汉字还原数量对比"
    x-axis ["本项目", "tabula"]
    y-axis "还原汉字数" 0 --> 22000
    bar [20177, 16995]
```


```mermaid
pie title tabula 汉字还原情况
    "tabula 还原" : 16995
    "tabula 丢失 " : 3182
```
